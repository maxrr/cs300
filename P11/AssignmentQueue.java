...
public class AssignmentQueue implements PriorityQueueADT<Assignment>, Iterable<Assignment> {
  ...
  public Assignment peek() {
    if (this.isEmpty())
      throw new NoSuchElementException("Cannot peek if the queue is empty!");
   ...
   * @throws IllegalStateException with a descriptive error message if this Assign mentQueue is full
   */
  @Override
  public void enqueue(Assignment e) {
    if (e == null)
      throw new NullPointerException("The provided Assignment is null");
    if (this.size == this.queue.length)
      throw new IllegalStateException("The queue is full, no new Assignments can be added");
    ...
    this.queue[this.size - 1] = e;

    // then percolate it up.
  ...
  public Assignment dequeue() {
    ...
    Assignment prevRoot = this.queue[0];

    // move last element to first, then...
  ...
  protected void percolateDown(int i) {
    ...
    // check if left child would be within array limits, if it is compare this element to it
    // initialize this to zero so that if left child doesn't exist,
    int iToLeft = 0;
    if (i * 2 + 1 < this.size)
      iToLeft = this.queue[i].compareTo(this.queue[i * 2 + 1]);

    // initialize to -1 so we will always use left child if we can (if left child doesn't exist then
    // right child doesn't exist)
    int leftToRight = -1;

    // check if right child would be within array limits, if it is compare this element to it
    int iToRight = 0;
    if (i * 2 + 2 < this.size) {
      iToRight = this.queue[i].compareTo(this.queue[i * 2 + 2]);
      leftToRight = this.queue[i * 2 + 1].compareTo(this.queue[i * 2 + 2]);
    }

    // if either child is greater
    if (iToLeft > 0 || iToRight > 0) {
      if (leftToRight > 0) {
        // left child is larger than right child (replace i with i's right child)
        Assignment temp = this.queue[i];
        ...
        // right child is larger than (or equal to) left child (replace i with i's left child)
        Assignment temp = this.queue[i];
        this.queue[i] = this.queue[i * 1 + 1];
        this.queue[i * 1 + 1] = temp;
  ...
  protected void percolateUp(int i) {
    ...
    // if we've gotten back to the root node, stop here
    if (i == 0)
      return;

    if (this.queue[i].compareTo(this.queue[(i - 1) / 2]) < 0) {
      // element at index i is less than parent, so they need to be replaced
      Assignment temp = this.queue[i];
      this.queue[i] = this.queue[(i - 1) / 2];
      this.queue[(i - 1) / 2] = temp;
  ...
  public AssignmentQueue deepCopy() {
    // make a new AssignmentQueue
    AssignmentQueue a = new AssignmentQueue(this.capacity());

    // enqueue all the same Assignments
    for (int i = 0; i < this.size; i++) {
      a.enqueue(this.queue[i]);
    }

    // return the new AssignmentQueue
    return a;
  }
